...
class SearchForm extends React.Component<SearchFormProps, SearchFormState> {
    ...
    collapse = (): void => this.setState({ collapse: !this.state.collapse })

    render() {
        return (
            <>
                <MediaQuery styles={{ display: 'none' }} largerThan={'md'}>
                    <Button onClick={this.collapse} sx={{ marginBottom: '1rem', width: '100%' }} color='gray'>
                        {this.state.collapse ? 'open search criteria' : 'collapse search criteria'}
                    </Button>
                </MediaQuery>
                <Transition mounted={!this.state.collapse} transition='scale-y' duration={400} timingFunction='ease'>
                    {(styles) =>
                        <Paper style={styles} sx={{ backgroundColor: '#1f2023' }} padding='md' shadow='sm' withBorder>
